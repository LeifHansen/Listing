...
| A failed location lookup no longer reads as "you have none" | *this commit* | `account_overview` swallowed every ship-from location failure -- a timeout, a 401, an eBay outage -- into `[]`, and Settings rendered that as "No inventory locations found." It is the same shape as the `programs_known` tri-state two fields away, with more riding on it: publishing needs a ship-from location, so a seller told they have none goes to eBay and creates a SECOND one for an account that already had it. `locations_known` now separates "eBay said none" from "eBay didn't answer". |

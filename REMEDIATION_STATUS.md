...
- [ ] P1-08 auth baseline, MINUS the headers (`46b89d3`) and the script-src
      tightening (see above): still 30-day irrevocable JWTs, no
      reset/verify/MFA, and process-local rate limiting. `style-src` keeps
      `'unsafe-inline'`, which is deliberate and documented, not outstanding.

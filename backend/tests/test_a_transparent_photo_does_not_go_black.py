...
tool's export was handed to the matte (and to the paid engines, which bill per
image) as an item on a black field.
...
def test_the_cutout_stage_is_handed_an_opaque_photo(tmp_path, monkeypatch):
    ...
    and what a per-image API is paid to look at."""
    seen = {}

    def _spy(img):
        seen["mode"] = img.mode
        seen["corner"] = img.convert("RGB").getpixel((2, 2))
        return img, "none", "no engine", False, None, None

    monkeypatch.setattr(images, "_studio_and_cutout", _spy)
